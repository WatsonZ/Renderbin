<script lang="ts">
	...
	import { register, AuthApiError } from '$lib/api/auth';
	...
	const { form, errors, enhance, submitting } = superForm(defaults(zod(registerSchema)), {
		...
		async onUpdate({ form }) {
			...
			errorMessage = null;
			try {
				await register(form.data.username, form.data.nickname, form.data.password);
				await goto(resolve('/'));
			} catch (err) {
				if (err instanceof AuthApiError && err.status === 409) {
					errorMessage = t('register.usernameTaken');
				} else if (err instanceof AuthApiError && err.status === 403) {
					errorMessage = t('register.disabled');
				} else {
					errorMessage = t('register.failed');
				}
			}
		}
	});
	...
	<div class="w-full max-w-sm">
		...
		>
			{#if !data.allowRegistration}
				<h1 class="text-lg font-semibold text-neutral-100">{t('register.title')}</h1>
				<p class="mt-1 mb-6 text-sm text-neutral-500">{t('register.disabled')}</p>
				<a
					href={resolve('/login')}
					class="flex items-center justify-center gap-2 rounded-lg bg-emerald-500 py-2.5 text-sm
						font-medium text-neutral-950 transition-colors hover:bg-emerald-400"
				>
			...
			{:else}
				<h1 class="text-lg font-semibold text-neutral-100">{t('register.title')}</h1>
				<p class="mt-1 mb-6 text-sm text-neutral-500">{t('register.subtitle')}</p>
